...
const Navbar:React.FC = () => {
  ...
  return (
    <nav className={styles.navbar}>
      <Link href= '/' ><a className= {styles["navbar__logo"]}>GamesTech<FaGamepad className= {styles.joystick}/></a></Link>
      {/* menu will appear reaching certain media query*/}
      <div className= {styles["menu-icon"]} onClick= {()=> setClick(!clicked)} >
        {clicked ? <AiOutlineClose/> : <AiOutlineMenu/>}
      </div>
      {/* navgation list */}
      <ul className= {clicked ? styles["menu-active"] : styles["menu-default"]}>
